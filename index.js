...
function manipulateSearchParams(paramsObj, newUrl) {
  ...
  let paramKeys = Object.keys(paramsObj)
...
function manipulateQuery(url, options) {
  let newUrl = new URL(url)
      ...
      if (options.has('append')) {
        ...
        for (let [appKey, appValue] of options.get('append')) {
        newUrl.searchParams.append(appKey, appValue)
        }
      }

      if (options.has('delete')) {
         
        for (let delKey of options.get('delete')){
          newUrl.searchParams.delete(delKey)
...
function sortUrlParams(url) {
  ...
  // let arr = [...newUrl.searchParams.entries()]
  // arr.sort();
  // newUrl.searchParams.clear();
...
function getUrlKeys(url) {
  let newUrl = new URL(url)
  let arr = [...newUrl.searchParams.keys()]
  return arr
  // Створюємо новий об'єкт URL з вхідної URL-адреси.
